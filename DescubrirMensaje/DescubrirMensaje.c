...
int isValid(UserData user) {
    // Check id
    for (int i = 0; i < strlen(user.id); i++) {
        if (!isalnum(user.id[i])) {
            return 0;
        }
    }
    ...
    // Check email
    if (sscanf(user.email, "%*[^@]@%*[^.].%*s") != 2) {
        return 0;
    }

    // Check age
    if (strlen(user.age) > 0 && !isdigit(user.age[0])) {
        return 0;
    }
...
int main() {
    // Definir las entradas de la base de datos
    UserData database[] = {
    ...
    // Encontrar usuarios inválidos y obtener el primer caracter del username
    char hiddenMessage[50] = "";
    for (int i = 0; i < sizeof(database) / sizeof(database[0]); i++) {
        if (!isValid(database[i])) {
            char firstChar = isalnum(database[i].username[0]) ? database[i].username[0] : '_';
            strncat(hiddenMessage, &firstChar, 1);
        }
    }

    // Imprimir el mensaje oculto
    printf("Mensaje oculto: %s\n", hiddenMessage);
